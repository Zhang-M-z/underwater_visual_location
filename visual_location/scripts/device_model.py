...
class DeviceModel:
    ...
    def __init__(self, deviceName, portName, baud, addrLis, callback_method,print_frequency):
        print("初始化设备模型")
    ...
    def openDevice(self):
        ...
        try:
            ...
            # 清除之前的初始值
            self.initial_angle_z.clear()
            self.is_initialized.clear()

            # 开启一个线程持续监听串口数据 Start a thread to continuously listen to serial port data
            self.t1 = threading.Thread(target=self.readDataTh, args=("Data-Received-Thread", 10,))
            self.t1.start()
            print("设备打开成功")
    ...
    def processData(self, length):
        ...
        if length == 30:
            ...
            # 解析角度（0x3D~0x42，每个角度32位）
            # AngX（0x3D-0x3E）
            ang_x_low = self.TempBytes[21] << 8 | self.TempBytes[22]
            ang_x_high = self.TempBytes[23] << 8 | self.TempBytes[24]
            AngX = self.getSignInt32((ang_x_high << 16) | ang_x_low) / 1000 # ±180°
        
            # AngY（0x3F-0x40）
            ang_y_low = self.TempBytes[25] << 8 | self.TempBytes[26]
            ang_y_high = self.TempBytes[27] << 8 | self.TempBytes[28]
            AngY = self.getSignInt32((ang_y_high << 16) | ang_y_low) / 1000 # ±90°
        
            # AngZ（0x41-0x42）
            ang_z_low = self.TempBytes[29] << 8 | self.TempBytes[30]
            ang_z_high = self.TempBytes[31] << 8 | self.TempBytes[32]
            AngZ = self.getSignInt32((ang_z_high << 16) | ang_z_low) / 1000 # ±180°

            # 检查并保存初始值
            if ADDR not in self.is_initialized or not self.is_initialized.get(ADDR, False):
                self.initial_angle_z[ADDR] = AngZ
                self.is_initialized[ADDR] = True
            
            self.set(ADDR, "AngX", round(AngX, 3))
            self.set(ADDR, "AngY", round(AngY, 3))
            # self.set(ADDR, "AngZ", round(AngZ, 3))
            # 应用初始值修正
            adjusted_angz = AngZ - self.initial_angle_z.get(ADDR, 0)
            adjusted_angz = (adjusted_angz + 180) % 360 - 180  # 确保在±180范围内
            self.set(ADDR, "AngZ", round(adjusted_angz, 3))
    ...
    def getSignInt32(num):
        if num >= pow(2, 31):
            num -= pow(2, 32)
        return num - 0x100000000 if num > 0x7FFFFFFF else num
    ...
    def startLoopRead(self):
        ...
        self.t2 = threading.Thread(target=self.loopRead)
        self.t2.start()
